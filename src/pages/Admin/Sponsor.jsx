...
export default function Sponsor() {
  ...
  const handleSponsorImageSubmit = async (event) => {
    ...
    if (
      !sponsorImage ||
      !["image/jpeg", "image/png", "image/gif"].includes(sponsorImage.type)
    ) {
      setSponsorImageMessage(
    ...
    <div className="sponsor-images-section">
      ...
      <form className="sponsor-image-form" onSubmit={handleSponsorImageSubmit}>
        ...
        <input
          ...
          className="sponsor-image-input"
          onChange={(e) => setSponsorImage(e.target.files[0])}
        />
        <button type="submit" className="submit-sponsor-image">
          Ajouter l'image
        </button>
      </form>
      {sponsorImageMessage && (
        <div
          className={`message ${
            sponsorImageIsError ? "message-error" : "message-success"
          }`}
        >
          {sponsorImageMessage}
        </div>
