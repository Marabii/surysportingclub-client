...
export default function ClubImages() {
  ...
  const handleClubImageSubmit = async (event) => {
    ...
    if (
      !clubImage ||
      !["image/jpeg", "image/png", "image/gif"].includes(clubImage.type)
    ) {
  ...
  return (
    <div className="club-images-section">
      <h2>Ajouter une image de club</h2>
      <form className="club-image-form" onSubmit={handleClubImageSubmit}>
        ...
        />
        <button type="submit" className="submit-club-image">
          Ajouter une image de club
        </button>
      </form>
      {clubImageMessage && (
        <div
          className={`message ${
            clubImageIsError ? "message-error" : "message-success"
          }`}
        >
          {clubImageMessage}
        </div>
