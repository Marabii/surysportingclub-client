...
export default function TeamsPage() {
  ...
  return (
    <div className="teams-page">
      <img src={background} alt="Dynamic Soccer Background" className="background-image-teams" />
      <Header />
      <h1>Equipes</h1>
        ...
        <div className="gender-section">
          ...
          <div>
            {maleTeams.map((team, index) => (
              <Link key={index} to={`/equipes/${team.name}-${team.gender}`} className="team-button">
                <div>{team.name}</div>
              </Link>
            ))}
          </div>
